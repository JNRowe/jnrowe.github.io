...
Many windows define a ``WM_COMMAND`` property that is set to the command name
of the application, if that exists all we have to do is read it::
...
The ``wmctrl`` output has five space aligned columns.  The first is the window
...
with the ``xprop`` example we must select a window with the mouse when calling
``xprop``, and then we pluck the process id from the output.
...
find its process id and send it a ``SIGTERM``.  It seems like a reasonable
idea, but I’ve been choosing the ``urxvtd`` examples on purpose to prove
a point.
