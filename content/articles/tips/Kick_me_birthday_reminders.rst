...
practically anything with only a small Python_ script using
...
Now the birthday trick is a cool one, ``abook`` allows the use of custom fields
that you can use for anything you like.  I use a custom field to store birthdays
in |ISO|-8601 format as in “1974-04-12”, and enable it by adding the following
to my ``~/.abook/abookrc``:
...
A little ``python`` script is all that is needed to generate a ``remind`` input
