...
The easiest method we use is via `shelldoctest
<https://pypi.python.org/pypi/shelldoctest>`_, a module that implements
a ``doctest`` interface for testing shell commands.  The package provides
...
For example, the `blockdiag <https://pypi.python.org/pypi/blockdiag>`_ ebuild
is accompanied by a series of tests that are run when bumping or stabilising
the package, all of which are cribbed from my actual ``blockdiag`` usage.  This
massively reduces the time required to evaluate a package at bump time.
...
how to combine the ``doctest`` module’s ``doctest.testfile`` function with
``shelldoctest``::
