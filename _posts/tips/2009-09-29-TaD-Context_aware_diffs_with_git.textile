...
{% comment %}
This nastiness is because jekyll is failing with code that
contains a '*' for some unknown(at this time) reason
{% endcomment %}
<div class="highlight"><pre><span class="k">[diff “ini”]</span>
	<span class="na">funcname</span> <span class="o">=</span> <span class="s">“^\\[.*\\]$”</span>
<span class="k">[diff “adr”]</span>
	<span class="na">funcname</span> <span class="o">=</span> <span class="s">“^#.*$”</span>
</pre></div>

And, to enable them you must tell @git@ which files to the new matchers with:

<div class="highlight"><pre>*.ini diff=ini
*.adr diff=adr
</pre></div>

The @funcname@ values are simple regular expressions to search for, so in the @ini@ example it searching for a line that begins with a @[@ and ends with a @]@ as these are the common section headers.  And the @adr@ matcher just specifies a line that begins with a @#@.  It is important to match the entire string, as it is the matched content that is used in the diff hunk's output.
