...
The following examples use an older version of @be@ that you can download as a "tarball":http://www.jnrowe.ukfsn.org/_static/be-0.0.193.tar.bz2 and don't require @bzr@.  As stated above, if you're using "Gentoo":http://www.gentoo.org/ you can use the @dev-util/be@ ebuild in my "overlay":http://github.com/jnrowe/misc-overlay.
...
{% highlight text %}
0 jay@kate ~/Desktop (master) $ mkdir be_test; cd be_test
1 jay@kate ~/Desktop/be_test (master) $ git init
Initialized empty Git repository in /home/jay/Desktop/be_test/.git/
1 jay@kate ~/Desktop/be_test (master) $ be set-root
...
We can easily file new bugs, in the next snippet we can see two bugs being filed.  Bugs are identified by a short UUID(Universally Unique IDentifier), and to operate on bugs we only need to use a unique prefix of the identifier as can be seen below.

{% highlight text %}
0 jay@kate ~/Desktop/be_test (master) $ be new "This is a test bug"
Created bug with ID a09
0 jay@kate ~/Desktop/be_test (master) $ be assign a09
0 jay@kate ~/Desktop/be_test (master) $ git commit -m"Commit bug a09."
0 jay@kate ~/Desktop/be_test (master) $ be new "This is a second bug"
Created bug with ID ec4
0 jay@kate ~/Desktop/be_test (master) $ be severity ec4 serious
1 jay@kate ~/Desktop/be_test (master) $ be comment ec4 "Comments are easy"
1 jay@kate ~/Desktop/be_test (master) $ git commit -m"Commit bug ec4."
{% endhighlight %}
...
{% highlight text %}
0 jay@kate ~/Desktop/be_test (master) $ be list
...
{% highlight text %}
b0 jay@kate ~/Desktop/be_test (master) $ be show a09
...
0 jay@kate ~/Desktop/be_test (master) $ be show ec4
...
{% highlight text %}
0 jay@kate ~/Desktop/be_test (master) $ be status ec4 fixed
0 jay@kate ~/Desktop/be_test (master) $ be list
a09:om: This is a test bug
0 jay@kate ~/Desktop/be_test (master) $ be show ec4
          ID : ec4438ca-a330-4345-b073-43c768f7e9b7
