...
if __name__ == "__main__":
    ...
    parse(addressbook)
{% endhighlight %}
p>. "Fork this code":http://gist.github.com/
...
The script opens either the name or default @abook@ addressbook and fetch any entries that contain both a @nick@ and @email@ section.  Those entries are then output in format that @sup@ accepts.  It includes only the primary email address for the contact, as I tend to order contacts with multiple email addresses in address preference order.
