...
p(note). There is another important point here too as the example above uses character classes, just as Rob's original did.  As in most RE(Regular Expression) engines Python has inbuilt support for certain character classes, and they are often more functional than the simple character classes.  The big win with Python's character classes is that they can be locale and/or Unicode aware by specifying the "LOCALE":http://docs.python.org/library/re.html#re.LOCALE or "UNICODE":http://docs.python.org/library/re.html#re.LOCALE flags.

These types of string escaping problems are common, and as your RE(Regular Expression) becomes more complex their likelihood increases greatly.  Python, as is often the case, has the answer and it comes in the shape of the @DEBUG@ flag which unfortunately appears not be documented.  The @DEBUG@ flag displays a parse tree allowing you to easily inspect the RE(Regular Expression) you have constructed:
...
Another exceptional tool for tasks people often abuse RE(Regular Expressions) for is "python-dateutil":http://labix.org/python-dateutil.  One of the reason I occasionally turn to this module is that Python's "time.strptime":http://docs.python.org/library/time.html#time.strptime isn't even capable of parsing timestamps created by "time.strftime":http://docs.python.org/library/time.html#time.strftime, for example if you use @%z@ in the @strftime@ call.
